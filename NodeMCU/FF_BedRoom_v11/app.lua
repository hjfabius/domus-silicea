local app = {}
    ...
    function app.refreshAndSend()
        ...
        if config.GPIO_PIR >= 0 then
            PIR.update()
            if PIR.Status == 1 then 
                MQTT.send_Message("V_TRIPPED",  "OPEN"   )
            elseif PIR.Status == 0 then 
                MQTT.send_Message("V_TRIPPED",  "CLOSED"   )
            end
        end

        if config.GPIO_Light >= 0 then
            Light.update()
            if Light.Status == 1 then 
                MQTT.send_Message("V_LIGHT",  "ON"   )
            elseif Light.Status == 0 then 
                MQTT.send_Message("V_LIGHT",  "OFF"   )
            end
        end
        ...
        if config.GPIO_ArduinoOneWire >= 0 then
            arduinoOneWire.start()
        end 
